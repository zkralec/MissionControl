...
DEFAULT_DISCORD_USER_AGENT = "MissionControlDiscordNotifier/1.0"
...
def _post_once(url: str, payload: dict[str, Any]) -> tuple[int, Any, str | None]:
    user_agent = os.getenv("DISCORD_USER_AGENT", DEFAULT_DISCORD_USER_AGENT).strip()
    if not user_agent:
        user_agent = DEFAULT_DISCORD_USER_AGENT
...
def send_discord_webhook(content: str, metadata: dict | None = None) -> dict:
    ...
    details = ""
    if response_body:
        details = f"; response={response_body[:240]}"
    raise RuntimeError(f"discord webhook failed with status {status_code}{details}")
