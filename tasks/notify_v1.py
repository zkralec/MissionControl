...
class NotifyTaskError(Exception):
    """notify_v1 error for non-retryable failures."""
...
def _default_dedupe_ttl() -> int:
    ...
    try:
        ...
        raise NotifyTaskError("NOTIFY_DEDUPE_TTL_SECONDS must be an integer") from exc
    if ttl < 1:
        raise NotifyTaskError("NOTIFY_DEDUPE_TTL_SECONDS must be >= 1")
    return ttl
...
def execute(task: Any, db: Any) -> dict[str, Any]:
    try:
        ...
        raise NotifyTaskError(f"notify_v1 payload is invalid JSON: {exc.msg}") from exc
    ...
    if not isinstance(source_task_type, str) or not source_task_type.strip():
        raise NotifyTaskError("notify_v1 requires payload.source_task_type")

    allowlist = _parse_allowlist()
    if source_task_type not in allowlist:
        raise NotifyTaskError(
    ...
    if ttl_seconds < 1:
        raise NotifyTaskError("dedupe_ttl_seconds must be >= 1")
    ...
    for channel in channels:
        ...
        try:
            ...
            raise NotifyTaskError(f"notify_v1 send failed for channel '{channel}'") from exc
